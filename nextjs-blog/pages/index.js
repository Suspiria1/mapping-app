...
export default function Home() {
    const calculateScalingFactor = (baseWidth, baseHeight) => {
        const userScreenWidth = window.innerWidth;
        const userScreenHeight = window.innerHeight;

        // Calculate scaling factors for width and height
        const widthFactor = baseWidth / userScreenWidth;
        const heightFactor = baseHeight / userScreenHeight;

        const scalingFactor = Math.min(widthFactor, heightFactor);

        return scalingFactor;
    };

    const baseWidth = 1535; // Predetermined base width
    const baseHeight = 585; // Predetermined base height
    ...
    const circleStyle = {
        position: "absolute",
        top: `${circlePosition.y}px` * heightFactor,
        left: `${circlePosition.x}px` * widthFactor,
